...
VOID ApplyComputerOwnersPolicy(IN PLATFORM* sys)
{
  ...
  for (UINTN pidx = 0; pidx < sys->PkgCnt; pidx++) {
    ...
    pk->planes[RING].OffsetVolts = -50;// -50;        // in mV 
                                                    // (negative = undervolt)
    ...
    pk->planes[ECORE].OffsetVolts = -50;          // in mV (negative = undervolt)
    ...
    pk->planes[UNCORE].OffsetVolts = -50;// -35;  // in mV (negative = undervolt)
