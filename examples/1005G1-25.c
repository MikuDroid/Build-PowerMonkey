...
VOID ApplyComputerOwnersPolicy(IN PLATFORM* sys)
{
  ...
  for (UINTN pidx = 0; pidx < sys->PkgCnt; pidx++) {
    ...
    pk->planes[RING].OffsetVolts = -25;// -50;        // in mV 
                                                    // (negative = undervolt)
    ...
    pk->planes[ECORE].OffsetVolts = -25;          // in mV (negative = undervolt)
    ...
    pk->planes[UNCORE].OffsetVolts = -25;// -35;  // in mV (negative = undervolt)
