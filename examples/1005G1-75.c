...
VOID ApplyComputerOwnersPolicy(IN PLATFORM* sys)
{
  ...
  for (UINTN pidx = 0; pidx < sys->PkgCnt; pidx++) {
    ...
    pk->planes[RING].OffsetVolts = -75;// -50;        // in mV 
                                                    // (negative = undervolt)
    ...
    pk->planes[ECORE].OffsetVolts = -75;          // in mV (negative = undervolt)
    ...
    pk->planes[UNCORE].OffsetVolts = -75;// -35;  // in mV (negative = undervolt)
